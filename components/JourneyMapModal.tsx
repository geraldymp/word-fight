...
export const JourneyMapModal = ({
    ...
    const updatedJourney: INode[][] = [[{ name: 'Start', chosen: true, type: "other" as const }], ...journey, [{ name: '???', chosen: false, type: "other" as const }]].reverse();
    const renderNode = (node: INode) => {
        if (node.type === 'enemy') {
            ...
            return (
                <Text style={styles.nodeOther}>
                    {node.name}
                </Text>
            );
        } else if (node.type === 'booster') {
            return (
                <Text style={styles.nodeBooster}>
                    {node.name}
                </Text>
            );
        }
                ...
                <View style={styles.modalContainer}>
                    <ScrollView contentContainerStyle={styles.scrollContent}>
                        {updatedJourney.map((level, rowIndex) => {
                            ...
                            const justifyContent = level.length > 1 ? 'space-between' : 'center';
                            const flex = level.length > 1 ? 2 : 0;
                            return (
                                <View key={rowIndex} style={{ width: '100%' }}>
                                    {showLine && <View style={{ height: 20, width: 2, backgroundColor: 'white', alignSelf: 'center' }} />}
                                    <View style={[styles.row, { justifyContent }]}>
                                        {level.map((node, colIndex) => {
                                            return (
                                                <View key={colIndex} style={[styles.nodeWrapper, { flex }]}>
